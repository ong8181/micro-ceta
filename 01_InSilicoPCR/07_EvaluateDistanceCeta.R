...
for (primer_i in primer_tested) {
  ...
  amplicon <- readDNAStringSet(sprintf("01_usearchOut/1_EachPrimerSet/CheckEditDistance_Cetacea/%s/hits_all.fa", primer_i))
...
ggsave(sprintf("%s/VariabilityCetacea.pdf", output_folder),
       plot = g1, width = 12, height = 10)
ggsave(sprintf("%s/VariabilityCetacea2.pdf", output_folder),
       plot = g2, width = 12, height = 10)
ggsave(sprintf("%s/VariabilityCetacea3.pdf", output_folder),
       plot = g3, width = 12, height = 10)
ggsave(sprintf("%s/VariabilityCetacea4_slide.pdf", output_folder),
       plot = g1 + facet_wrap(. ~ primer, nrow = 3), width = 14, height = 7)

# For poster
ggsave(sprintf("%s/VariabilityCetacea_Long.pdf", output_folder),
       plot = g1 + facet_wrap(. ~ primer, ncol = 4),
       width = 8, height = 12)
