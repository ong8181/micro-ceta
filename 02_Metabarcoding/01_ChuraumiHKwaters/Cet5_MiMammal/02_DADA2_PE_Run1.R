...
output_folder <- "02_DADA2Out/Run1_Out"
dir.create("02_DADA2Out")
dir.create(output_folder)

# Load sequence reads
path <- paste0(getwd(), "/01_QualityFiltering_Run1Out")
system("rm 01_QualityFiltering_Run1Out/unknown*")
fnFs <- sort(list.files(path, pattern="R1.fastq.gz", full.names = T)) # Forward read files
fnRs <- sort(list.files(path, pattern="R2.fastq.gz", full.names = T)) # Reserve read files
# Get sample names
...
data("primer_set")
# Identify primers
FWD <- primer_set$Animal12S_MiMammal$forward
REV <- primer_set$Animal12S_MiMammal$reverse
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs,
                     truncLen=c(120, 120), # Some low quality seuqences are found at the end
                     #truncLen=c(210, 200), # For 16S 505F-806R, Truncate the end of reads
...
min_nbases <- 240 * sum(out[,2]) # Use a small number of bases to speed up the analysis
