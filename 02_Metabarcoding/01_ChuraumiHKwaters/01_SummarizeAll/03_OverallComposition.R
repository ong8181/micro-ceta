...
ps_mt1$bundled_tax[ps_mt1$species == "Homo sapiens"] <- "Homo sapiens"
ps_mt1$bundled_tax[ps_mt1$family == "Hominidae"] <- "Homo sapiens"
ps_mt1$bundled_tax[ps_mt1$assignSp_Species == "sapiens"] <- "Homo sapiens"
ps_mt1$bundled_tax[ps_mt1$species == "Canis lupus"] <- "Canis lupus"
ps_mt1$bundled_tax[ps_mt1$species == "Sus scrofa"] <- "Sus scrofa"
ps_mt1$bundled_tax[ps_mt1$species == "Bos taurus"] <- "Bos taurus"
ps_mt1$bundled_tax[ps_mt1$superclass == "Actinopterygii"] <- "Actinopterygii"
...
ps_mt1$bundled_tax <- factor(ps_mt1$bundled_tax,
                             levels = c("Cetacea","Bacteria","Homo sapiens","Sus scrofa","Bos taurus",
                                        "Canis lupus","Actinopterygii",
                                        "Other Eukaryota","Others","Undetermined"))
...
f1 <- ggplot(ps_mt2, aes_(x = as.name("new_name2"), y = as.name("sequence_reads"),
...
# --------------------------------------------- #
# Save figure
ggsave(sprintf("%s/Amplified_tax.pdf", output_folder), f1, width = 12, height = 6)
